...
- 1성 → 2성 → 3성 자동 합성(총 9장)
- [아이템 패치](docs/ITEM_PATCH_2026-09-10.md): 주문검·3연타·성장·지원 효과와 특수 장비 선택 보상
- 재료 10종 + 조합 결과 55종 + 유물 6종 + 찬란한 6종 = **아이템 77종**, 유닛당 최대 3슬롯
- **공통 특성 24종**(각질 / 거리·주로 / 역사·세대), 인자·서포트 카드로 만드는 엠블럼 16종
- **증강체 84종**(Silver / Gold / Prism) — 전용 스킬 강화·영구 성장·아이템 기록, 판마다 공통 등급 추첨
- **레이스 플랜 90종** — 2-5 출주 계획 → 3-5 전개 수정 → 4-5 GⅠ 출주 등록 → 최종 승부수.
  보드·아이템·경제·최근 전투를 읽어 플레이어마다 다른 3장을 제시한다.
  [설계 문서](docs/RACE_PLAN_CODEX_SPEC_v2.md) · [패치 노트](docs/RACE_PLAN_PATCH_2026-09-13.md)
- 헥스 보드 7 × 8, 30초 전투 + 15초 오버타임
- 시즌: **5개 × 출전 60명**, 시즌 간 중복 허용으로 **145명 전원 출전**. 새 게임과 온라인 방 생성에서 선택.
- 시너지: 기존 특성 24종에 **시즌 전용 20종** 추가(시즌당 4종, 3/5/7명 단계). [시즌별 명단·효과](docs/SEASONS.md).
- 도감: 전체 145명의 출전 시즌, 시즌별 시너지와 스킬 확인·필터.
...
145명의 명시적 스킬 프로필을 42개 전투 패턴으로 구성했습니다. 대표 경기 재선정, 거리 특성 12건 교정, 연쇄·직선·부채꼴 이펙트 및 시전 프레임 타이밍을 적용했습니다. [기물별 변경과 근거](docs/SKILL_PATCH_2026-09-10.md).

증강·각질·AI 및 우클릭/드래그 입력 패치: [변경 내역과 검증](docs/qa/augment-expansion-patch.md).
