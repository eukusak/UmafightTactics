...
import { ACTIVE_S1_SIZE } from '../../game/engine/constants';
...
export function OnlineScreen(): JSX.Element {
  ...
  return <div className="menu-screen online-screen season-screen" style={seasonBackdrop(online.room?.seasonId ?? seasonId)}>
    ...
    {!online.room ? <div className="online-connect panel">
      ...
      <label>새 방 시즌<select value={seasonId} onChange={e => setSeasonId(e.target.value as SeasonId)}>{SEASONS.map(s => <option key={s.id} value={s.id}>{s.id.toUpperCase()} · {s.name} · {ACTIVE_S1_SIZE}명</option>)}</select></label>
      ...
      <p className="gold-text">{online.room.seasonId.toUpperCase()} · {getSeason(online.room.seasonId).name} · 출전 {ACTIVE_S1_SIZE}명</p>
