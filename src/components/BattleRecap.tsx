...
export function BattleRecap(): JSX.Element {
  ...
  const [raceLog, setRaceLog] = useState(false);
  ...
  return <div className="battle-recap"><h3>전투 통계 · {running ? '현재 전투' : '이전 전투'}</h3>
    <div className="recap-tabs"><button aria-pressed={!raceLog} onClick={() => setRaceLog(false)}>기여도</button><button aria-pressed={raceLog} onClick={() => setRaceLog(true)}>레이스 플랜</button></div>
    {raceLog ? <ol className="race-recap-log">{frames.slice(0, frameAt(frames,time)+1).flatMap(f => f.events).filter(e => e.type === 'RACE_PHASE' || e.type === 'RACE_PROC').map((e,i) => <li key={i}><b>{e.t.toFixed(1)}초</b> {e.type === 'RACE_PHASE' ? `${RACE_PHASE_LABEL[e.phase]} · ${Math.round(e.progress*100)}%` : e.type === 'RACE_PROC' ? `${(frame.units.find(u=>u.id===e.unit) ? getUnitDef(frame.units.find(u=>u.id===e.unit)!.unitDefId).nameKo : '기물')} · ${e.label}${e.stacks ? ' '+e.stacks+'중첩' : ''}` : ''}</li>)}</ol> : <>
