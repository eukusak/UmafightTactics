...
export function SeasonPicker({ value, onChange }: { value: SeasonId; onChange: (id: SeasonId) => void }): JSX.Element {
  ...
  return <section className="season-picker" aria-label="시즌 선택">
    <div className="season-picker-heading"><strong>이번에는 어떤 무대로?</strong><span>5개 시즌 · 시즌당 {ACTIVE_S1_SIZE}명 · 전체 {CANONICAL_ROSTER_SIZE}명 출전</span></div>
    ...
    <div className="season-details" style={{ '--season-color': season.color } as React.CSSProperties}>
      <div className="season-detail-heading"><strong>{season.name} · 전용 시너지</strong><button className="btn-ghost" type="button" onClick={() => setRosterOpen(true)}>출전 기물 {ACTIVE_S1_SIZE}명 보기</button></div>
    ...
    {rosterOpen && <div className="overlay" onClick={() => setRosterOpen(false)}><div className="overlay-card season-roster scroll" onClick={e => e.stopPropagation()}>
      <div className="season-detail-heading"><h2>{season.name} · {ACTIVE_S1_SIZE}명</h2><button onClick={() => setRosterOpen(false)}>닫기</button></div>
