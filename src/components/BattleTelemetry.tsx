...
export function BattleTelemetry(): JSX.Element | null {
  ...
  const damagePanel = <button type="button" aria-label="전투 기여 전체 보기" className={`battle-damage-panel${running ? '' : ' previous'}`} onClick={() => useInteractionStore.getState().inspect({ kind: 'recap' })}>
