...
export const TRAIT_DEFS: TraitDef[] = [
  ...
  },
  {
    ...
    tiers: [
      ...
      { count: 4, description: '체력 60% 이상일 때 피해 증폭 +15%. 첫 스킬 마나 -10',
        effects: [{ kind: 'DAMAGE_AMP', value: 0.15, trigger: { when: 'HP_ABOVE', threshold: 0.6 } },
          { kind: 'MANA_ADD', value: 10, trigger: { when: 'COMBAT_START' } }] },
      { count: 6, description: '체력 60% 이상일 때 피해 증폭 +25%. 첫 스킬 마나 -15',
        effects: [{ kind: 'DAMAGE_AMP', value: 0.25, trigger: { when: 'HP_ABOVE', threshold: 0.6 } },
  ...
  },
  {
    ...
    tiers: [
      ...
      { count: 4, description: '치명타 +10%. 처치 관여 시 6초간 추가 치명타 +15%, 피해 증폭 +5%',
        effects: [{ kind: 'CRIT_CHANCE_ADD', value: 0.1 }, { kind: 'CRIT_CHANCE_ADD', value: 0.15, duration: 6, trigger: { when: 'ON_TAKEDOWN_ASSIST' } },
          { kind: 'DAMAGE_AMP', value: 0.05, duration: 6, trigger: { when: 'ON_TAKEDOWN_ASSIST' } }] },
      { count: 6, description: '치명타 +15%. 처치 관여 시 6초간 추가 치명타 +25%, 피해 증폭 +10%',
        effects: [{ kind: 'CRIT_CHANCE_ADD', value: 0.15 }, { kind: 'CRIT_CHANCE_ADD', value: 0.25, duration: 6, trigger: { when: 'ON_TAKEDOWN_ASSIST' } },
