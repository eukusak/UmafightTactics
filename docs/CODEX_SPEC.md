...
- 완성 아이템은 다시 분해 불가
- 같은 완성 아이템 중복 가능, 단 `unique` 또는 같은 `uniqueGroup`은 중복 장착 불가. 주문검 3종 및 일반/찬란한 같은 계열에도 적용한다.
- 전투 중 필드 유닛에게 장착 금지
- 준비 단계/벤치에는 장착 가능
...
| ID | 이름 | 효과 |
|---|---|---|
| `winner_ribbon` | **우승자 리본** | 공격력 +5, 체력 +40. |
| `reinforced_horseshoe` | **강화 편자** | 방어력 +15, 체력 +40. |
| `training_belt` | **트레이닝 벨트** | 체력 +180. |
| `tactics_notebook` | **작전 노트** | 주문력 +12, 시작 마나 +3. |
| `spurt_band` | **스퍼트 밴드** | 공격속도 +12%. |
| `focus_drop` | **집중의 물방울** | 시작 마나 +10. 3초마다 마나 +2. |
| `weather_cloak` | **비바람 망토** | 마법저항력 +15, 체력 +40. |
| `race_glove` | **레이스 글러브** | 치명타 확률 +15%. |
| `factor_badge` | **인자 배지** | 능력치 없음. 특성 부여 재료. |
| `support_card` | **서포트 카드** | 능력치 없음. 특성 부여 재료. |

## 23.3 전체 55 조합

| ID | 조합 | 이름 | 효과 |
|---|---|---|---|
| `champion_trophy` | 우승자 리본 + 우승자 리본 | **연승 사냥꾼의 트로피** | 공격력 +20%. 처치 관여마다 공격력 +6%, 전투당 최대 4중첩. |
| `twilight_racing_suit` | 우승자 리본 + 강화 편자 | **삼박자 승부복** | 방어력 +20, 체력 +100, 공격력 +10%. 스킬 사용 후 5초 내 다음 기본 공격에 공격력 80% 추가 물리 피해(2초 재사용). 공격 후 2초간 이동속도 +20%, 중첩 없이 갱신. |
| `unyielding_fighting_spirit` | 우승자 리본 + 트레이닝 벨트 | **불굴의 승부근성** | 체력 +300, 공격력 +10%. 체력이 처음 35% 아래가 되면 최대 체력 30% 보호막(4초). 군중제어 지속시간 20% 감소. |
| `trainer_lifeblade` | 우승자 리본 + 작전 노트 | **동행의 생명검** | 주문력 +20, 공격력 +10%. 모든 피해 흡혈 14%. 기본 공격 3회마다 자신 외 체력 비율이 가장 낮은 아군 65 회복. |
| `giant_overtaker` | 우승자 리본 + 스퍼트 밴드 | **거인 추월자** | 공격력 +10%, 공격속도 +20%. 기본 공격 적중 후 대상 현재 체력 4.5% 추가 물리 피해(최대 100). 모든 피해 흡혈 8%. |
| `start_dash_plan` | 우승자 리본 + 집중의 물방울 | **축적형 스타트 작전** | 시작 마나 +15, 공격력 +10%. 공격마다 마나 +3. 스킬 사용마다 공격력 +8%, 전투당 최대 4중첩. |
| `victory_bloodwind` | 우승자 리본 + 비바람 망토 | **갈라진 결승선** | 체력 +150, 마법저항력 +20, 공격력 +10%. 대상별 6초마다 기본 공격에 공격력 55% 추가 물리 피해와 자신의 잃은 체력 15% 회복. |
| `finish_line_strike` | 우승자 리본 + 레이스 글러브 | **결승선의 일격** | 치명타 확률 +20%, 공격력 +15%. 치명타 피해 +30%. 스킬도 치명타가 발생한다. |
| `iron_stable` | 강화 편자 + 강화 편자 | **가시 편자의 마굿간** | 방어력 +50. 기본 공격 피격 시 공격자에게 25 + 방어력 40% 마법 피해(2초 재사용). 공격자에게 4초간 치유 감소 33%. |
| `heated_training_blanket` | 강화 편자 + 트레이닝 벨트 | **잿불 훈련 담요** | 방어력 +20, 체력 +250. 2초마다 주변 1칸 모든 적에게 3초 화상: 초당 최대 체력 0.8% 고정 피해와 치유 감소 33%. |
| `trainer_crownguard` | 강화 편자 + 작전 노트 | **개화하는 크라운가드** | 방어력 +25, 주문력 +25. 스킬 사용 시 최대 체력 12% 보호막(4초, 4초 재사용). 시전마다 주문력 +6, 전투당 최대 5중첩. |
| `iron_horseshoe_resolve` | 강화 편자 + 스퍼트 밴드 | **반격의 철편자** | 방어력 +25, 공격속도 +20%. 기본 공격 피격 시 최대 체력 8% 보호막(4초 재사용)과 3초간 공격속도 +30%. 공격속도는 중첩 없이 갱신. |
| `pre_race_vow` | 강화 편자 + 집중의 물방울 | **혹한의 출전 맹세** | 방어력 +25, 체력 +150, 시작 마나 +15. 적에게 군중제어를 실제 적용하면 최대 체력 16% 보호막(4초). 4초 재사용. 면역으로 막힌 제어에는 발동하지 않는다. |
| `racecourse_stoneplate` | 강화 편자 + 비바람 망토 | **장기전 석갑** | 방어력 +25, 마법저항력 +25. 전투 시작 6초 후 방어력과 마법저항력 +25를 추가 획득한다. 전투당 1회. |
| `steadfast_heart` | 강화 편자 + 레이스 글러브 | **굳건한 하트** | 방어력 +20, 체력 +200. 받는 피해 8% 감소. 체력 50% 미만에서는 총 18% 감소. |
| `long_distance_training_coat` | 트레이닝 벨트 + 트레이닝 벨트 | **강철심장 훈련 코트** | 체력 +400. 4초마다 기본 공격 시 최대 체력 +45(전투당 최대 6중첩), 증가한 최대 체력 3.5% 추가 물리 피해. 성장치는 다음 전투에 초기화. |
| `burning_spirit_strategy` | 트레이닝 벨트 + 작전 노트 | **타오르는 투지의 작전서** | 체력 +200, 주문력 +25. 스킬 피해를 받은 적에게 4초 화상: 초당 최대 체력 1% 고정 피해와 치유 감소 33%. 전투 6초 후 스킬 피해 +8%. |
| `pace_up_snack` | 트레이닝 벨트 + 스퍼트 밴드 | **실험적 페이스 보급식** | 체력 +200, 공격속도 +20%. 스킬 사용 후 4초간 공격속도 +40%. 반복 시전은 중첩 없이 지속시간만 갱신. |
| `recovery_saddle` | 트레이닝 벨트 + 집중의 물방울 | **달빛 회복의 안장** | 체력 +200, 시작 마나 +15. 4초마다 자신 외 체력 비율이 가장 낮은 아군의 최대 체력 7%를 회복한다. |
| `evening_race_armor` | 트레이닝 벨트 + 비바람 망토 | **쇠약의 야간 갑주** | 체력 +250, 마법저항력 +25. 2초마다 주변 2칸 적의 방어력과 마법저항력을 20% 감소(2.1초). 범위를 벗어나면 곧 해제된다. |
| `frontline_retake_mallet` | 트레이닝 벨트 + 레이스 글러브 | **선두 탈환 메달** | 체력 +250, 치명타 확률 +15%, 공격력 +10%. 4초마다 기본 공격에 공격력 40% + 대상 잃은 체력 4% 추가 물리 피해(최대 150). |
| `genius_trainer_hat` | 작전 노트 + 작전 노트 | **완성형 트레이너 모자** | 주문력 +50. 스킬 사용마다 주문력 +10, 전투당 최대 4중첩. |
| `endless_spurt` | 작전 노트 + 스퍼트 밴드 | **내셔의 스퍼트 노트** | 주문력 +30, 공격속도 +20%. 기본 공격마다 15 + 주문력 22% 추가 마법 피해. |
| `accumulated_fighting_spirit` | 작전 노트 + 집중의 물방울 | **영겁의 투지** | 체력 +150, 주문력 +20, 시작 마나 +15. 4초마다 주문력 +10, 최대 체력 +35. 전투당 최대 5중첩이며 다음 전투에 초기화. |
| `gate_shock_device` | 작전 노트 + 비바람 망토 | **메아리 게이트 충격기** | 주문력 +25, 마법저항력 +20. 스킬 적중 시 대상과 주변 1칸 적에게 30 + 주문력 35% 추가 마법 피해. 3초 재사용. |
| `jewel_race_glove` | 작전 노트 + 레이스 글러브 | **그림자 보석 글러브** | 주문력 +30, 치명타 확률 +20%. 스킬 치명타 가능. 현재 공격 대상의 체력이 40% 이하면 스킬 피해 +15%. |
| `red_turf_booster` | 스퍼트 밴드 + 스퍼트 밴드 | **삼연격 터프 부스터** | 공격속도 +35%. 기본 공격 3회마다 35 + 공격력 60% + 대상 잃은 체력 2.5% 추가 물리 피해(최대 180). 대상 변경 후에도 공격 횟수 유지. |
| `corner_piercer` | 스퍼트 밴드 + 집중의 물방울 | **갈래바람 관통봉** | 시작 마나 +15, 공격속도 +20%. 2초마다 기본 공격 대상과 주변 1칸 모든 적에게 공격력 30% 추가 물리 피해. |
| `breakaway_horseshoe` | 스퍼트 밴드 + 비바람 망토 | **파죽지세 편자** | 마법저항력 +25, 공격속도 +20%. 같은 적을 연속 3회 공격할 때마다 40 + 공격력 50% 추가 마법 피해와 자신의 체력 35 회복. 대상 변경 시 횟수 초기화. |
| `last_overtake` | 스퍼트 밴드 + 레이스 글러브 | **칠흑의 추월 편자** | 치명타 확률 +15%, 공격속도 +20%. 기본 공격마다 대상 방어력 6% 감소(5초, 최대 5중첩 30%). 같은 출처는 지속시간 갱신, 다른 방어력 감소와는 가장 큰 값만 적용. |
| `blue_focus` | 집중의 물방울 + 집중의 물방울 | **마나 순환 결정** | 시작 마나 +30, 주문력 +15. 스킬 사용 후 마나 10 회복. 처치 관여 시 마나 15 추가 회복(2초 재사용). |
| `adaptive_headgear` | 집중의 물방울 + 비바람 망토 | **겨울 집중 헤드기어** | 시작 마나 +15, 마법저항력 +30, 체력 +150. 3초마다 마나 +5. 스킬 사용 시 최대 체력 10% 보호막(4초, 3초 재사용). |
| `hand_of_victory` | 집중의 물방울 + 레이스 글러브 | **구원의 손길** | 시작 마나 +15, 치명타 확률 +15%, 주문력 +20. 스킬 사용 시 자신 외 체력 비율이 가장 낮은 아군의 잃은 체력 18% 회복(3초 재사용). 모든 피해 흡혈 10%. |
| `stormproof_racing_cloak` | 비바람 망토 + 비바람 망토 | **정령의 방풍 마의** | 마법저항력 +55, 체력 +200. 받는 회복과 보호막 +25%. 3초마다 자신의 최대 체력 2.5% 회복. |
| `composure_ribbon` | 비바람 망토 + 레이스 글러브 | **평정의 리본** | 마법저항력 +25, 치명타 확률 +15%, 공격속도 +20%. 전투 시작 6초간 군중제어 면역. 이후 군중제어 지속시간 35% 감소. |
| `trick_strategy_gloves` | 레이스 글러브 + 레이스 글러브 | **변칙 작전 글러브** | 아이템 슬롯 3칸 사용. 매 준비 단계 종료 시 완성 아이템 2개를 무작위로 장착. |
...
| `trainer_crown` | 인자 배지 + 인자 배지 | **트레이너 왕관** | 전략가 전용. 팀 최대 규모 +1. 유닛에게 장착하지 않는다. |
...
유물은 4-7 PvE 승리 시 6종 중 1개를 선택한다. 찬란한 장비는 「찬란한 레이스 장비」 증강에서 6종 중 1개를 선택한다. 준비 단계 아이템 보관함에서 수령하며, 보관함이 가득 차면 선택권을 보존한다. 일반 조합과 변칙 작전 글러브의 무작위 완성 장비 목록에는 포함하지 않는다.

| ID | 이름 | 효과 |
|---|---|---|
| `artifact_echo_lantern` | **유물 · 메아리 등불** | 주문력 +45, 시작 마나 +20. 스킬 적중 시 대상과 주변 1칸에 45 + 주문력 50% 마법 피해(3초 재사용). 처치 시 마나 +15(2초 재사용). |
| `artifact_moon_chime` | **유물 · 월석 공명종** | 주문력 +30, 체력 +200, 시작 마나 +20. 자신의 스킬로 다른 아군을 실제 회복시키거나 보호막을 주면, 자신 외 체력 비율이 가장 낮은 아군 100 회복(2초 재사용). 아이템 회복은 연쇄 발동하지 않는다. |
| `artifact_winter_oath` | **유물 · 겨울의 서약** | 방어력 +35, 마법저항력 +35, 체력 +200. 적에게 군중제어를 실제 적용하면 최대 체력 20% 보호막(4초, 3초 재사용). |
| `artifact_hydra_saddle` | **유물 · 히드라 안장** | 체력 +350, 공격력 +15%. 1.5초마다 공격 대상과 주변 1칸 모든 적에게 공격력 35% + 자신의 최대 체력 1.2% 물리 피해. |
| `artifact_duelist_clock` | **유물 · 결투자의 시계** | 주문력 +35, 시작 마나 +15, 공격력 +15%. 스킬 사용 후 6초 내 다음 기본 공격에 공격력 70% + 주문력 40% 추가 마법 피해(2초 재사용). 삼박자 승부복 계열과 동시 장착 불가. |
| `artifact_legacy_heart` | **유물 · 계승의 심장** | 체력 +450. 3초마다 공격 시 최대 체력 +60, 전투당 최대 8중첩. 스킬 사용 시 잃은 체력 12% 회복(3초 재사용). 성장치는 다음 전투에 초기화. |
| `radiant_twilight_racing_suit` | **찬란한 · 삼박자 승부복** | 방어력 +35, 체력 +200, 공격력 +20%. 스킬 사용 후 6초 내 다음 기본 공격에 공격력 115% 추가 물리 피해(2초 재사용). 공격 후 2초간 이동속도 +30%, 중첩 없이 갱신. |
| `radiant_giant_overtaker` | **찬란한 · 거인 추월자** | 공격력 +20%, 공격속도 +30%. 기본 공격 적중 후 대상 현재 체력 6% 추가 물리 피해(최대 150). 모든 피해 흡혈 12%. |
| `radiant_long_distance_training_coat` | **찬란한 · 강철심장 훈련 코트** | 체력 +600. 4초마다 공격 시 최대 체력 +60(전투당 최대 6중첩)과 최대 체력 5% 추가 물리 피해. 성장치는 다음 전투에 초기화. |
| `radiant_recovery_saddle` | **찬란한 · 달빛 회복의 안장** | 체력 +300, 시작 마나 +25. 4초마다 자신 외 가장 다친 아군의 최대 체력 10% 회복. 이어 현재 가장 다친 다른 아군에게 70 보호막(3초). |
| `radiant_red_turf_booster` | **찬란한 · 삼연격 터프 부스터** | 공격속도 +50%. 기본 공격 3회마다 50 + 공격력 90% + 대상 잃은 체력 3% 추가 물리 피해(최대 250). 대상 변경 후에도 횟수 유지. |
| `radiant_burning_spirit_strategy` | **찬란한 · 타오르는 투지의 작전서** | 체력 +300, 주문력 +40. 스킬 피해를 받은 적에게 5초 화상: 초당 최대 체력 1.2% 고정 피해와 치유 감소 33%. 전투 6초 후 스킬 피해 +12%. |

[2026-09-10 아이템 패치 설계](ITEM_PATCH_2026-09-10.md)
...
| `item_radiant` | P | **찬란한 레이스 장비** | 찬란한 장비 6종 중 1개 선택. 각 장비별 강화 수치와 재사용 시간을 적용한다. |
