...
> **2026-09-07 사용자 지시 우선 적용:** 아래 도트 스타일 지시는 [새 아트 방향과 실제 납품 현황](ART_DIRECTION_2026.md)으로 대체한다. 매끈한 셀 셰이딩·Live2D 느낌의 2.5D 표현을 목표로 하며, 파일/격자 규격은 유지한다. 현재 납품 규격 통과는 **220/514**, 캐릭터 자산 294개는 미완료다. 아래의 PRODUCTION READY는 발주서의 과거 상태이며 전체 이미지 제작 완료를 뜻하지 않는다.
