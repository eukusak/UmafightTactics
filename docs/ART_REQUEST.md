...
> **2026-09-07 사용자 지시 우선 적용:** 아래 도트 스타일 지시는 [새 아트 방향과 실제 납품 현황](ART_DIRECTION_2026.md)으로 대체한다. 매끈한 셀 셰이딩·Live2D 느낌의 2.5D 표현을 목표로 하며, 파일/격자 규격은 유지한다. 현재 납품 규격 통과는 **222/514**, 캐릭터 자산 292개는 미완료다. 아래의 PRODUCTION READY는 발주서의 과거 상태이며 전체 이미지 제작 완료를 뜻하지 않는다.
...
- 전투 애니메이션 실제 프레임: **캐릭터당 46프레임**
- 전체 캐릭터 전투 프레임: **6670프레임**
...
> **2026-09-08 추가 지시:** 피격 동작은 사용하지 않는다. 피해 이벤트로 자세 전환·움찔·밀림·몸 크기 변화·공격 중단을 만들지 않는다. row 4 col 0~3은 호환용 예약 셀로 비워 두며, 새 피격 원화 제작 대상에서 제외한다. 공격·이동·스킬·전투 불능·승리 동작은 유지한다. 피해는 체력/보호막 게이지와 숫자로만 표시한다.
...
### hit 4 — 사용 중단
- 제작하지 않음. 기존 격자 호환용 예약 셀.
- 피해 이벤트는 공격/스킬/이동 모션에 영향을 주지 않음.
...
- 셀 **128×128** / **10열 × 4행**
- idle 6 / attack 8 / reserved 4 (투명) / ko 6
- 남는 셀 투명
